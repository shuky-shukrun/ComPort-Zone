"""Tests for the control_panel CSV value logger (FR-49/FR-50/FR-51)."""
...
from ComPort_Zone.core.control_panel import (
    VALUE_LOG_FIELDS,
...
class ControlPanelValueLoggerTests(unittest.TestCase):
    def setUp(self) -> None:
        self._tmp = TemporaryDirectory()
        self.addCleanup(self._tmp.cleanup)
        self.tmp = Path(self._tmp.name)
        self.logger = ControlPanelValueLogger()
    ...
    def test_log_row_round_trips_with_value_number(self) -> None:
        ...
        row = rows[0]
        self.assertEqual(row["control_panel"], "Bench")
...
class ControlPanelValueLoggerKindColumnTests(unittest.TestCase):
    ...
    def test_pre_v3_log_without_kind_reads_back(self) -> None:
        ...
        with path.open("w", encoding="utf-8-sig", newline="") as handle:
            handle.write(
                "timestamp,control_panel,entry_id,label,value_text,value_number,state\n"
            )
