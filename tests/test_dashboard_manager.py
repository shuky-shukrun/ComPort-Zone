"""Tests for the dashboard manager dialog."""
...
import ComPort_Zone.ui.dialogs.dashboard_manager as manager_module
from ComPort_Zone.dashboard_catalog import DashboardCatalog
from ComPort_Zone.dashboard_models import DashboardConfig, DashboardEntry
from ComPort_Zone.ui.dialogs.dashboard_manager import DashboardManagerDialog


def make_config(name: str) -> DashboardConfig:
    return DashboardConfig(name=name, entries=[DashboardEntry(label="V", command="MEAS?")])


class DashboardManagerDialogTests(unittest.TestCase):
    @classmethod
    ...
    def make_dialog(self) -> DashboardManagerDialog:
        def save() -> None:
            self.saves += 1

        return DashboardManagerDialog(
            catalog=self.catalog,
            open_dashboard=self.opened.append,
            close_dashboard_tab=self.closed_tabs.append,
            save_settings=save,
            set_status=self.statuses.append,
        )

    def test_lists_dashboards_sorted_with_entry_counts(self) -> None:
    ...
    def test_duplicate_selected_saves_and_refreshes(self) -> None:
        ...
        self.assertEqual(len(self.dashboards), 2)
    ...
    def test_delete_declined_keeps_dashboard(self) -> None:
        self.catalog.add(make_config("Bench"))
        dialog = self.make_dialog()
        original_question = QMessageBox.question
        manager_module.QMessageBox.question = staticmethod(
    ...
    def test_rename_selected_dedupes(self) -> None:
        ...
        for row in range(dialog.list_widget.count()):
            if str(dialog.list_widget.item(row).data(manager_module.ROLE_DASHBOARD_ID)) == target.id:
