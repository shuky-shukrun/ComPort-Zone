"""Tests for the dashboard run coordinator (binding + dispatcher refcounts)."""
...
class DashboardRunCoordinatorTests(unittest.TestCase):
    @classmethod
    ...
    def test_acquire_attaches_traffic_journal_release_detaches(self) -> None:
        ...
        self.assertIs(session.dashboard_traffic_journal, dispatcher.traffic_journal)
        self.coordinator.release_dispatcher(1)
        self.assertIsNone(session.dashboard_traffic_journal)
    ...
    def test_shared_acquire_keeps_one_journal(self) -> None:
        ...
        self.assertIs(session.dashboard_traffic_journal, first.traffic_journal)
        self.coordinator.release_dispatcher(1)
        # One dashboard still bound — the journal must stay attached.
        self.assertIs(session.dashboard_traffic_journal, first.traffic_journal)
        self.coordinator.release_dispatcher(1)
        self.assertIsNone(session.dashboard_traffic_journal)
    ...
    def test_refresh_dashboards_notifies_tabs(self) -> None:
        self.dashboards = [FakeDashboardTab(), FakeDashboardTab()]
        self.coordinator.refresh_dashboards()
        self.assertEqual([tab.refresh_count for tab in self.dashboards], [1, 1])
