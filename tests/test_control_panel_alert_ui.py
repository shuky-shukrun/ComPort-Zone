"""Tests for the control_panel alert UI surfaces (FR-58).
...
- ``QtAlertSounder`` debounces and falls back when QtMultimedia or the
  WAV file is missing.
- Preferences gets a ControlPanels tab whose checkboxes round-trip into
  ``AppSettings``.
...
from ComPort_Zone.ui.control_panel_alert_panel import AlertHistoryPanel
...
class AppSettingsAlertsFieldsTests(unittest.TestCase):
    def test_defaults_are_on_with_sound_off(self) -> None:
        ...
        self.assertTrue(settings.control_panel_alerts_enabled)
        self.assertFalse(settings.control_panel_alert_sound)

    def test_round_trip(self) -> None:
        original = AppSettings()
        original.control_panel_alerts_enabled = False
        original.control_panel_alert_sound = True
        restored = AppSettings.from_dict(original.to_dict())
        self.assertFalse(restored.control_panel_alerts_enabled)
        self.assertTrue(restored.control_panel_alert_sound)

    def test_missing_block_keeps_defaults(self) -> None:
        # An older settings file without the control_panel_alerts block must
        # not silently disable alerts on upgrade.
        payload = AppSettings().to_dict()
        del payload["app"]["control_panel_alerts"]
        restored = AppSettings.from_dict(payload)
        self.assertTrue(restored.control_panel_alerts_enabled)
        self.assertFalse(restored.control_panel_alert_sound)


class PreferencesControlPanelsTabTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.qt = QApplication.instance() or QApplication([])
    ...
    def test_round_trip(self) -> None:
        settings = AppSettings()
        settings.control_panel_alerts_enabled = False
        settings.control_panel_alert_sound = True
        dialog = PreferencesDialog(settings)
        self.assertFalse(dialog.control_panel_alerts_checkbox.isChecked())
        self.assertTrue(dialog.control_panel_alert_sound_checkbox.isChecked())
        # Master toggle gates the sound checkbox so it's a clearly
        # secondary option.
        self.assertFalse(dialog.control_panel_alert_sound_checkbox.isEnabled())
        dialog.control_panel_alerts_checkbox.setChecked(True)
        self.assertTrue(dialog.control_panel_alert_sound_checkbox.isEnabled())

        dialog.control_panel_alerts_checkbox.setChecked(True)
        dialog.control_panel_alert_sound_checkbox.setChecked(False)
