"""Tests for dashboard tile widgets, grid geometry, and theming."""
...
def make_config(*entries: DashboardEntry, columns: int = 4) -> DashboardConfig:
    return DashboardConfig(name="Test", columns=columns, entries=list(entries))
...
class GridGeometryTests(unittest.TestCase):
    ...
    def make_grid(self, config: DashboardConfig, width: int = 800) -> DashboardGridWidget:
...
class ThemingTests(unittest.TestCase):
    ...
    def test_stylesheet_builds_with_dashboard_blocks_for_every_theme(self) -> None:
        for name, theme in THEMES.items():
            with self.subTest(theme=name):
                qss = build_stylesheet(theme)
                self.assertIn("QFrame#dashboardTile", qss)
                self.assertIn('QLabel#tileLamp[tileState="ok"]', qss)
                self.assertIn('QLabel#dashboardBindChip[state="polling"]', qss)

    def test_no_hardcoded_hex_colors_in_dashboard_widgets(self) -> None:
        hex_literal = re.compile(r"#[0-9a-fA-F]{6}\b")
        for module in ("ui/dashboard_tiles.py", "ui/dashboard_grid.py"):
