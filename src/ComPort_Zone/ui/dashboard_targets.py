"""Dashboard binding coordinator: terminal-session targets + dispatchers.

Sibling of :class:`~ComPort_Zone.ui.command_file_targets.CommandFileRunCoordinator`,
with two deltas: bind targets include *disconnected* sessions (a dashboard
...
lifecycle with reference counting so every dashboard bound to one session
shares one dispatcher (FR-17, NFR-4).

Requirements: docs/dashboard-view-requirements.md (FR-10..FR-17).
"""
...
from ..dashboard_engine import SessionPollDispatcher
...
class DashboardTabLike(Protocol):
    def refresh_binding_state(self) -> None:
        ...


@dataclass(frozen=True, slots=True)
class DashboardBindTarget:
    session_id: int
    label: str
    endpoint: str
    connected: bool
...
class SessionHealth:
    ...
    subscribes to the old one; the bound dashboard must rebind.
    """
...
class DashboardRunCoordinator:
    """MainWindow-level service shared by all dashboard tabs."""

    def __init__(
        self,
        *,
        sessions_supplier: Callable[[], Iterable[TerminalSessionLike]],
        dashboards_supplier: Callable[[], Iterable[DashboardTabLike]],
        is_widget_open: Callable[[object], bool],
        set_status: Callable[[str], None],
        target_icon_color: Callable[[], str],
    ) -> None:
        self._sessions_supplier = sessions_supplier
        self._dashboards_supplier = dashboards_supplier
    ...
    def _set_session_journal(session: TerminalSessionLike, journal) -> None:
        attach = getattr(session, "attach_dashboard_traffic_journal", None)
        if callable(attach):
            attach(journal)
    ...
    def refresh_dashboards(self) -> None:
        """Ask every open dashboard tab to re-render its binding chip
        (called from MainWindow.update_connection_status)."""
        for dashboard in self._dashboards_supplier():
            dashboard.refresh_binding_state()
