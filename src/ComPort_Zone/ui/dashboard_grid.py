"""Dashboard grid: manual tile geometry with spans and drag-to-place.
...
Qt-free layout math in ``dashboard_models``. The grid mutates the
``DashboardConfig`` it was given via ``place_tile``/``set_tile_span`` and
...
class DashboardGridWidget(QWidget):
    """Renders one dashboard config as a grid of tiles."""
    ...
    @property
    def config(self) -> DashboardConfig | None:
        return self._config
    ...
    def _drag_entry_id(self, event) -> str:
        data = event.mimeData().data(DASHBOARD_TILE_MIME_TYPE)
        return bytes(data).decode("utf-8") if data else ""

    def dragEnterEvent(self, event: QDragEnterEvent) -> None:  # noqa: N802
        if self._edit_mode and event.mimeData().hasFormat(DASHBOARD_TILE_MIME_TYPE):
