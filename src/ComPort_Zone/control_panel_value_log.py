"""Per-control_panel CSV value logger (FR-49/FR-50/FR-51).

A small, Qt-free sibling of :class:`ComPort_Zone.session_log.SessionLogger`
for the durable record of control_panel polls. Each successful parse — poll
...
control_panel itself (:class:`ControlPanelConfig.csv_log_enabled` /
``csv_log_path``) so it survives both restarts and control_panel reloads.
...
class ControlPanelValueLogger:
    """Append-only CSV sink for one control_panel's poll/derived parses."""
