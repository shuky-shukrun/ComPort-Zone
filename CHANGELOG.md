...
- Control Panel **visual cleanup**: smaller tile corner radius, tighter tile margins and grid gutter, and shorter minimum row height for a denser, clearer grid. Input tiles (control / setpoint / enum) now render **at least 2 columns wide** so their spinbox/dropdown/button aren't clipped on the dense default grid.
