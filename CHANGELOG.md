...
- Settings schema bumped to **v8** for static text tiles. Panels using only v1 fields keep their v5 floor; v2-only panels keep their v6 floor; v3 panels keep their v7 floor; only a panel that actually uses a static tile declares the v8 floor — so older builds correctly detect (and refuse) a panel they'd otherwise misread, instead of silently turning the note into a broken value tile. Export JSON stamps `version: 4` only when the payload actually uses a static tile; v1/v2/v3 exports still stamp 1/2/3.
