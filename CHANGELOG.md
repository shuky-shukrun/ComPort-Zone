...
- **LED caption and status-bit labels scale with tile size too** — the same latent issue as the value reading: their font was computed from the cell size but applied with a plain `setFont`, which the per-poll repolish (the lamp/caption state change, or a bit's active-state toggle) silently reset, so in the running app those labels stayed at the default size regardless of how big the tile was. The scaled size now rides each label's inline stylesheet and persists.
