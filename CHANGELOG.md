...
- Control Panel **visual cleanup**: smaller tile corner radius, tighter tile margins and grid gutter, and shorter rows for a denser, clearer grid. Input widgets are more compact — the setpoint field drops its up/down arrows (numeric validation/clamping kept) and inputs sit closer to their border — so a control/setpoint/enum tile reads cleanly even at one column wide.
