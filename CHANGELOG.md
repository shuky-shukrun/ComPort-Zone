...
- **Duplicate Control Panel** is now on the panel tab's right-click menu (it was already in the Control Panels manager). Duplicating remaps follow-mode `watch_entry_id` references to the copied tiles, so readbacks in the copy point at the copy.
