...
- **Text / note tiles** — a new tile kind that just shows an (optional) title over a wrapped block of explanatory text. No command, no polling, no arming — purely for labelling and documenting a panel (section headings, operator instructions, safety limits). Pick **Text / note** in the tile editor; the Polling and Response tabs disappear and a multi-line **Text** field appears. An untitled note shows only its body.
- **Separator / divider tiles** — another static tile kind that draws a horizontal rule to visually section a panel. Give it a label to get a centered caption (`──── Outputs ────`); leave it blank for a plain full-width line. Like text tiles it has no data exchange.
...
- Settings schema bumped to **v8** for static text/separator tiles. Panels using only v1 fields keep their v5 floor; v2-only panels keep their v6 floor; v3 panels keep their v7 floor; only a panel that actually uses a static tile declares the v8 floor — so older builds correctly detect (and refuse) a panel they'd otherwise misread, instead of silently turning the tile into a broken value tile. Export JSON stamps `version: 4` only when the payload actually uses a static tile; v1/v2/v3 exports still stamp 1/2/3.
