...
- **Control Panel v3.0**: the Dashboard View feature graduates to **Control Panel**, an industrial-grade HMI surface — same engine, broader write side, gated by a master arm. v1+v2 user settings load byte-for-byte (internal symbols and JSON keys keep their `dashboard*` names; only user-visible labels move).
  - **Numeric setpoint tile** — a new tile kind with a slider + double-spinbox bound to the same float setpoint state plus an optional readback that mirrors a watched polled tile (commanded vs measured side by side). Configurable min/max/step/decimals/unit and a `{value}`-templated command string (e.g. `VOLT {value}`). Out-of-range typed values clamp to bounds. Validation ensures `min < max`, `step > 0`, and the template contains exactly one `{value}` token (FR-63..FR-67).
  - **Enum / dropdown tile** — a new tile kind: a labeled `QComboBox` where each option carries its own send command. Optional `watch_entry_id` paints a subtle indicator pill on the option whose `match_value` matches the watched tile's reading, so operators see "current device mode" while the dropdown stays free to switch. Send fires through the same dispatcher FIFO as control + setpoint tiles (FR-68..FR-71).
  ...
  - **Audit CSV `kind` column** — `DashboardValueLogger` adds a `kind` column ∈ `poll` | `derived` | `control` between `label` and `value_text`. Control sends append a row on submit-result with the post-template command in `value_text` and `ok`/`error` in `state`. Old logs (no `kind` column) still read back fine — `csv.DictReader` treats missing columns as missing keys (FR-76..FR-77).
  - **Naming migration** — "Dashboard View" → "Control Panel" everywhere the user sees it: menus (`File > New Control Panel`, `Open Control Panel`), commands (`Control Panels…`, `Import/Export Control Panels`), sidebar rail page (Control Panels + Favorite Control Panels), manager dialog, Preferences tab, status-bar wording, README. Internal symbols (`DashboardConfig`, `DashboardTabWidget`, `AppSettings.dashboards`, `dashboard_value_log.py`, `tests/test_dashboard_*`, QSS object names) keep their names — your v1/v2 saved settings load with zero diff. `Ctrl+Shift+D` keeps its binding.
