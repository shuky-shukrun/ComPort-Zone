...
- **FR-72** Every Control Panel maintains a transient `armed` state that never persists to settings — it is **Disarmed** at every load (panel construction, tab restore, app launch) and force-disarmed after any unbind / target-session close. By default a panel then **arms itself when its bound device connects** (FR-74) so an operator does not have to click Arm every session; a per-panel persisted `view_only` flag opts out of that auto-arm (the panel stays disarmed for monitoring). `view_only` is sparse-serialized (written only when true).
...
- **FR-74** A panel arms automatically on the **connect edge** of its bound session (not on every tick), unless `view_only` is set — so a deliberate disarm during a live connection is respected, and a fresh reconnect re-arms. Arming is also explicit: the panel header carries an Arm/Disarm button (lock icon when disarmed, unlock icon when armed; accent-amber and accent-red surface colors respectively). Clicking Arm flips the state to Armed and emits an `armingChanged` signal that every writing tile observes to re-render. Enabling `view_only` while armed disarms immediately. **Per-tile `confirm` is NOT bypassed by arming** — arming just opens the gate; per-tile confirmation still fires.
