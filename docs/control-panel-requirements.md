...
- **FR-59** Tile kind `control` sends instead of polling. Modes: `button` (one command per click) and `toggle` (ON/OFF commands; the visual state follows optional readback verdict — `ok` renders ON — or an optimistic local flag when no readback is set). When a readback is configured and its reported ON/OFF state differs from the last commanded direction, the toggle adds a mismatch warning highlight (the device did not end up where the click asked). Optional per-tile confirmation shows the standard Yes/No prompt (default No) naming the command and target. Control entries are never scheduled, never stale, never alert, and keep no history.
