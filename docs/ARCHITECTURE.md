...
- Support serial, raw TCP, and UDP transports through the shared transport layer, while making future transports possible without rewiring the whole app.
...
| Transport abstraction | `transports.py`, `serial_core.py`, `lan_core.py`, `udp_core.py`, `transport_kinds.py` | Same | Done | Serial, raw TCP, and UDP adapters share the terminal controller/event contract and the same `PortChannel`. `transport_kinds.py` holds every user-visible name for a kind, so a fourth transport is one table row rather than a grep. The raw-TCP kind is persisted as `lan` (frozen by settings files on disk, and reflected in the `Lan*` class names) but is presented as **TCP** everywhere — TCP and UDP are both LAN transports, so "LAN" named the network instead of the protocol. Future transports add adapters, not UI branches. |
...
| Done | Transport abstraction foundation with serial, TCP, and UDP adapters | Raw TCP and UDP are implemented as client transports. |
