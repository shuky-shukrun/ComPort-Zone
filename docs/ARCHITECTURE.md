...
| Dashboard domain | `dashboard_models.py`, `dashboard_parse.py`, `dashboard_engine.py`, `dashboard_catalog.py` | Same | Done | Qt-free models, response parsing/rules, poll scheduler + per-session dispatcher, and the named-dashboard library; re-exported via `core/dashboard.py` (Qt-free enforced by tests). Requirements: `docs/dashboard-view-requirements.md`. |
| Dashboard UI | `ui/dashboard_tab.py`, `ui/dashboard_tiles.py`, `ui/dashboard_grid.py`, `ui/dashboard_targets.py`, `ui/dialogs/dashboard_entry.py`, `ui/dialogs/dashboard_manager.py` | Same | Done | Third workspace tab type: tick loop + binding chip, value/LED tiles, span-aware drag grid, binding coordinator with refcounted dispatchers, entry editor with live parse tester, library manager. |
