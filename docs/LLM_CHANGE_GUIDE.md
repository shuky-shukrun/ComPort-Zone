...
| --- | --- | --- |
| App startup, splash, compatibility re-exports | `app.py` | `tests.test_app_sessions` |
| Main window shell and high-level wiring | `ui/main_window.py` | `tests.test_app_sessions` |
| Top menu/action wiring | `ui/main_window_menus.py`, `command_registry.py` | `tests.test_main_window_menus`, `tests.test_command_registry` |
| Tab right-click menus | `ui/tab_context_menus.py` | `tests.test_tab_context_menus` |
| Command palette dynamic tab entries | `ui/command_palette_entries.py` | `tests.test_command_palette_entries` |
| Workspace tab lifecycle | `ui/tab_workspace.py` | `tests.test_tab_workspace`, app-session tests |
| Footer/tab status presentation | `ui/workspace_status.py` | `tests.test_workspace_status` |
| Terminal send/run/event behavior | `terminal_session_controller.py` | `tests.test_terminal_session_controller` |
| Terminal QTextEdit rendering/search | `terminal_view.py` | `tests.test_terminal_view` |
| Terminal tab UI layout/glue | `ui/terminal_tab.py` | terminal tests plus app-session tests |
| Serial behavior | `serial_core.py` | `tests.test_serial_core` |
| Transport abstraction | `transports.py` | `tests.test_transports` |
| Command-file parsing/running | `batch.py`, `command_file_service.py` | `tests.test_batch`, `tests.test_command_file_service` |
| Command-file editor UI | `command_editor.py` | `tests.test_command_editor` |
| Editor core/highlighting/search | `command_editor_core.py`, `command_editor_highlighting.py`, `command_search.py` | matching focused tests |
| Editor run target menus | `ui/command_file_targets.py` | `tests.test_command_file_targets` |
| Quick command/file domain | `quick_actions.py` | `tests.test_quick_actions` |
| Quick action workflows | `quick_action_controller.py` | `tests.test_quick_action_controller` |
| Quick action shared UI | `quick_actions_panel.py`, `quick_actions_sidebar.py` | matching quick action UI tests |
| Settings schema/import/export | `settings_service.py`, `storage.py`, `models.py` | `tests.test_models_and_storage` |
| Workspace settings save/restore | `workspace_state.py`, `workspace_settings_controller.py` | matching workspace tests |
| Dialog UI | `ui/dialogs/*` | `tests.test_dialogs` |

## Do Not Break These Seams

- `ComPort_Zone.app.MainWindow` must keep working even though the class lives in `ui/main_window.py`.
...
1. Edit `command_registry.py` if the command is shared or should appear in menus/palette.
2. If it is top-menu-specific, check `ui/main_window_menus.py`.
3. If it is tab-right-click-specific, check `ui/tab_context_menus.py`.
4. Add/update tests:
...
1. Edit `ui/tab_context_menus.py`.
...
1. Edit `MainWindow.rename_session()` in `ui/main_window.py`.
2. The dialog default should use visible tab text, not always internal `session.title`.
3. Keep `QInputDialog` imported in `ui/main_window.py`.
4. Run:
...
1. Add or update `CommandSpec` in `command_registry.py`.
...
1. Edit `ui/command_palette_entries.py`.
...
1. Start in `terminal_session_controller.py`.
2. Only edit `ui/terminal_tab.py` if the bug is UI input, message box, combo state, or focus behavior.
...
1. Parsing and batch semantics: edit `batch.py`.
2. Controller dispatch: edit `terminal_session_controller.py`.
3. Run target menu/target selection: edit `ui/command_file_targets.py`.
4. Parameter dialog: edit `ui/dialogs/command_file_parameters.py`.
5. Run likely tests:
...
1. Search state/pure logic: edit `command_search.py`.
2. Editor UI: edit `command_editor.py`.
3. Run:
...
1. Edit `command_editor_highlighting.py`.
...
1. Domain behavior: edit `quick_actions.py`.
2. Workflow/dialog behavior: edit `quick_action_controller.py`.
3. Shared panel/sidebar behavior: edit `quick_actions_panel.py` or `quick_actions_sidebar.py`.
4. Run:
...
1. Schema/payload: edit `settings_service.py` and `models.py`.
2. File I/O: edit `storage.py`.
3. Runtime tab capture/restore: edit `workspace_state.py`.
4. Live save/apply coordination: edit `workspace_settings_controller.py`.
5. Run:
...
- Do not duplicate quick action sidebar behavior between terminal and editor.
- Do not place top-menu or context-menu construction back into `MainWindow`.
...
Before finishing a minor code change:

- Focused tests for touched modules passed.
- Full `unittest discover -q` passed, unless explicitly documented why not.
- `git diff --check` passed.
- No unrelated refactors or formatting churn.
- Docs updated if ownership, flow, or design changed.
- Compatibility imports through `ComPort_Zone.app` still work if touched.
